...
public class KandangAActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_kandang_aactivity);
    ...
    private void CreateData(){
        ...
        Call<KandangAResponse> tambahData = ardData.ardTambahDataKandangA(tanggal, pakanTotal,
                sisa1, sisa2, sisa3, sisa4, sisa5, sisa6,jumlahTelur, beratTelur, jumlahMati, jumlahAfkir,
                suhuPagi, suhuSiang, suhuSore, nama);

        tambahData.enqueue(new Callback<KandangAResponse>() {
            @Override
            public void onResponse(Call<KandangAResponse> call, Response<KandangAResponse> response) {
                int kode = response.body().getKode();
                String pesan = response.body().getPesan();

                Toast.makeText(KandangAActivity.this, "Kode : " + kode +"| Pesan : " + pesan, Toast.LENGTH_SHORT).show();
                finish();
            }

            @Override
            public void onFailure(Call<KandangAResponse> call, Throwable t) {
                Toast.makeText(KandangAActivity.this, "Gagal menghubungi server" + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void simpan(){
        ...
        if (pakanTotal.trim().equals("")){
            ...
            if (cbYakin.isChecked()){
                ...
                new AlertDialog.Builder(KandangAActivity.this)
                        .setTitle("Peringatan")
