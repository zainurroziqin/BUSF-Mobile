...
public class KandangBActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_kandang_bactivity);
    ...
    private void CreateData(){
        ...
        Call<KandangBResponse> tambahData = ardData.ardTambahDataKandangB(tanggal, pakanTotal,
                sisa1, sisa2, sisa3, sisa4, sisa5, sisa6,jumlahTelur, beratTelur, jumlahMati, jumlahAfkir,
                suhuPagi, suhuSiang, suhuSore, nama);

        tambahData.enqueue(new Callback<KandangBResponse>() {
            @Override
            public void onResponse(Call<KandangBResponse> call, Response<KandangBResponse> response) {
                int kode = response.body().getKode();
                String pesan = response.body().getPesan();

                Toast.makeText(KandangBActivity.this, "Kode : " + kode +"| Pesan : " + pesan, Toast.LENGTH_SHORT).show();
                finish();
            }

            @Override
            public void onFailure(Call<KandangBResponse> call, Throwable t) {
                Toast.makeText(KandangBActivity.this, "Gagal menghubungi server" + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void simpan(){
        ...
        if (pakanTotal.trim().equals("")){
            ...
            if (cbYakin.isChecked()){
                ...
                new AlertDialog.Builder(KandangBActivity.this)
                        .setTitle("Peringatan")
