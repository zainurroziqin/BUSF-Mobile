...
public class KandangCActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_kandang_cactivity);
    ...
    private void CreateData(){
        ...
        Call<KandangCResponse> tambahData = ardData.ardTambahDataKandangC(tanggal, pakanTotal,
                sisa1, sisa2, sisa3, sisa4, sisa5, sisa6,jumlahTelur, beratTelur, jumlahMati, jumlahAfkir,
                suhuPagi, suhuSiang, suhuSore, nama);

        tambahData.enqueue(new Callback<KandangCResponse>() {
            @Override
            public void onResponse(Call<KandangCResponse> call, Response<KandangCResponse> response) {
                int kode = response.body().getKode();
                String pesan = response.body().getPesan();

                Toast.makeText(KandangCActivity.this, "Kode : " + kode +"| Pesan : " + pesan, Toast.LENGTH_SHORT).show();
                finish();
            }

            @Override
            public void onFailure(Call<KandangCResponse> call, Throwable t) {
                Toast.makeText(KandangCActivity.this, "Gagal menghubungi server" + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void simpan(){
        ...
        if (pakanTotal.trim().equals("")){
            ...
            if (cbYakin.isChecked()){
                ...
                new AlertDialog.Builder(KandangCActivity.this)
                        .setTitle("Peringatan")
